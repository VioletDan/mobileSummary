...
function importCom() {
	...
	com.validate = function  (obj,callback) {
		if(obj) {
			obj.find('.submit').on('click',sub_handler)
		} //end if
		...
		function sub_handler(){
			...
			inputs.each(function(index, el) {
				...
				if( ($(el).val()=='' && $(el).html()=='') ||  //非空
					( $(el).attr("name")=='tel'  && !icom.checkStr($(el).val()) ) ||		//电话
					( $(el).attr("name")=='email'  && !icom.checkStr($(el).val(),2) ) ||	//邮箱
					( $(el).attr("name")=='cardid'  && !icom.checkStr($(el).val(),3) )		//身份证
				){	//身份证
					var txt = $(el).data('txt');
					icom.alert(txt);
					return false;
				}
				if(index == inputs.length-1 && callback){
					 callback($(this));
				}
	 ...
	 * 微博授权
	 */
	com.weiboSign = function(callback) {
		if (navigator.userAgent.toLowerCase().indexOf('messenger') != -1) return;
		var apiHost = 'https://weibo-test.beats-digital.com';  
		$.ajax({
		  type: 'post',
		  url: apiHost + '/Api/Handler.ashx?method=Login',
		  dataType: 'json',
		  success: function(response) {
			...
			if (parseInt(response.errcode) == 0) {
			  if (response.result.authorized) {
				...
				callback && callback(response.result.data)
			  } else {
				console.log('authorized=false')
				var style = 'visibility:hidden;position:fixed;width:100%;height:100%;left:0;top:0;box-sizing: border-box;padding:15px;z-index:10000;background-color:rgba(0,0,0,0.5);color:#222;line-height:1;'
				var weiboSign = $('<div id="weiboSign" style="' +style +'">'+
									'<div style="position:relative;width:100%;height:100%;background-color:#fff">'+
									  '<iframe src="' +response.result.data.url +'" style="width:100%;height:100%;"></iframe>'+
									'</div>'+
								  '</div>');
				$('body').append(weiboSign);
				$('iframe',weiboSign).on('load', function(e){
				  setTimeout(function(){weiboSign.css({'visibility':'visible'});}, 1000);
				});
				
				function receiveMessage(event) {
				  if(event.origin.indexOf(apiHost) != -1 && event.data.indexOf('{') != -1) {                
					weiboSign.remove();
					console.log(event);
...
$("article").on("click","[data-link]",function  () {
 	...
 	setTimeout(function  () {
 		var lianjie =''
 		href.indexOf("?")>0?lianjie="&v=":lianjie="?v=";
 		location.replace(href+lianjie+Math.random())
 	},300)
});
/**
 * 处理ios输入框的问题
